            ...
            <a-card class="acard" v-for="(item, index) in animeList" hoverable>
                ...
                <div class="animeupdate">
                    <a-tooltip title="修改番剧详情">
                        <a-button @click="updateanime(item['aid'])" type="dashed" shape="circle" size="large">
                            <template #icon>
                                <edit-two-tone />
                            </template>
                        </a-button>
                    ...
                    <a-tooltip title="新增剧集">
                        <a-button @click="uploadanime(item['aid'])" type="dashed" shape="circle" size="large">
                            <template #icon>
...
const updateanime = async (aid: string) => {
    await fetch(`http://localhost:1314/api/anime/GetAnimeByID/${aid}`, {
        ...
        const _a_type: string = anime['type']
        anime['type'] = _a_type.split(',')
        if (anime['arecommend']) {
